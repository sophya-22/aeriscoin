...
updates will be released anymore. Without security updates, using a aeriscoin
...
This allows running aeriscoind without having to do any manual configuration.
...
aeriscoind can now (optionally) asynchronously notify clients through a
...
`aeriscoin.conf`, where `N` is the number of MiB to allot for
raw block & undo data.
...
caching. A sample config for apache2 could look like:
    ...
    <Location /aeriscoinrpc>
        ...
        # optional bypass aeriscoind rpc basic auth
        # RequestHeader set Authorization "Basic <hash>"
        # get the <hash> from the shell with: base64 <<< aeriscoinrpc:<password>
    </Location>
...
Restarting aeriscoind will no longer clear out the list of banned peers; instead
...
- Adding `dbcache=100` in aeriscoin.conf
...
aeriscoin-cli: arguments privacy
------------------------------
...
For example:
    ...
    $ src/aeriscoin-cli -stdin walletpassphrase
    mysecretcode
    120
...
- `aeriscoin-${VERSION}-arm-linux-gnueabihf.tar.gz`: Linux binaries for the most
  common 32-bit ARM architecture.
- `aeriscoin-${VERSION}-aarch64-linux-gnu.tar.gz`: Linux binaries for the most
...
The two phases are distinct in the Aeriscoin-Qt GUI. During the first one,
...
- **Signature covers value:** A simple improvement in the way signatures are
  ...
  of aeriscoins they think they are spending, and by having full nodes refuse to
  accept those signatures unless the amount of aeriscoins being spent is exactly
...
- `aeriscoin-${VERSION}-arm-linux-gnueabihf.tar.gz`: Linux binaries targeting
  the 32-bit ARMv7-A architecture.
- `aeriscoin-${VERSION}-aarch64-linux-gnu.tar.gz`: Linux binaries targeting
