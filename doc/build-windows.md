...
The next three steps are an example of how to acquire the source in an appropriate way.
    ...
    cd /usr/src
    sudo git clone https://github.com/aeriscoin-project/aeriscoin.git
    sudo chmod -R a+rw aeriscoin
...
way. This will install to `c:\workspace\aeriscoin`, for example:

    make install DESTDIR=/mnt/c/workspace/aeriscoin
...
causes two of the aeriscoin executables to crash shortly after start up. The bug is related to the
...
It's not possible to build the aeriscoin code using the win32 version of the Mingw-w64 cross compilers (at least not without
modifying headers in the aeriscoin source code).
