...
*
*   Why is this needed?
*   - In Aeriscoin some blocks are superblocks, which output much higher amounts of coins
    ...
    if (strCommand == NetMsgType::MASTERNODEPAYMENTSYNC) { //Masternode Payments Request Sync
        ...
        if(!vote.CheckSignature(mnInfo.pubKeyMasternode, pCurrentBlockIndex->nHeight, nDos)) {
            if(nDos) {
                ...
                if(masternodeSync.IsSynced()) Misbehaving(pfrom->GetId(), nDos);
            } else {
...
bool CMasternodeBlockPayees::IsTransactionValid(const CTransaction& txNew)
{
    ...
    LogPrintf("CMasternodeBlockPayees::IsTransactionValid -- ERROR: Missing required payment, possible payees: '%s', amount: %f ARS\n", strPayeesPossible, (float)nMasternodePayment/COIN);
    return false;
}
