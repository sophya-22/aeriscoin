...
const int MAX_OUTBOUND_MASTERNODE_CONNECTIONS = 25;
...
void CNode::copyStats(CNodeStats &stats)
{
    ...
    // Raw ping time is in microseconds, but show it to user as whole seconds (Aeriscoin users should be well used to small numbers with many decimal places by now :)
...
static bool AttemptToEvictConnection(bool fPreferNewConnection) {
    ...
    vEvictionCandidates.erase(vEvictionCandidates.end() - std::min(10, static_cast<int>(vEvictionCandidates.size())), vEvictionCandidates.end());
...
void ThreadMapPort()
{
    ...
    if (r == 1)
    {
        ...
        string strDesc = "Aeriscoin Core " + FormatFullVersion();
...
bool BindListenPort(const CService &addrBind, string& strError, bool fWhitelisted)
{
    ...
    if (::bind(hListenSocket, (struct sockaddr*)&sockaddr, len) == SOCKET_ERROR)
    {
        ...
        if (nErr == WSAEADDRINUSE)
            strError = strprintf(_("Unable to bind to %s on this computer. Aeriscoin Core is probably already running."), addrBind.ToString());
        else
