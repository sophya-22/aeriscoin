...
unsigned int static ARSDiff(const CBlockIndex* pindexLast, const Consensus::Params& params)
{
...
unsigned int static ARSDGW(const CBlockIndex* pindexLast, const Consensus::Params& params) {
    const CBlockIndex *BlockLastSolved = pindexLast;
    const CBlockIndex *BlockReading = pindexLast;
...
{
    unsigned int retarget = DIFF_ARS;
    if (Params().NetworkIDString() == CBaseChainParams::MAIN || Params().NetworkIDString() == CBaseChainParams::REGTEST) {
        ...
        else if (pindexLast->nHeight + 1 >= 45000) retarget = DIFF_ARST;
        ...
        else if (pindexLast->nHeight + 1 >= 10 && pindexLast->nHeight + 1 < 30) retarget = DIFF_ARS;
        else if (pindexLast->nHeight + 1 >= 30) retarget = DIFF_ARST;
        else retarget = DIFF_BTC;
    }
    ...
    else if (retarget == DIFF_ARS) return ARSDiff(pindexLast, params);
    else return ARSDGW(pindexLast, params);
