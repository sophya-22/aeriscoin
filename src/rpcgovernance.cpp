...
UniValue gobject(const UniValue& params, bool fHelp)
{
    ...
    if (fHelp  ||
        ...
        throw std::runtime_error(
            ...
            "  vote-conf          - Vote on a governance object by masternode configured in aeriscoin.conf\n"
    ...
    if(strCommand == "vote-conf")
    {
        ...
        if(eVoteSignal == VOTE_SIGNAL_NONE) {
            throw JSONRPCError(RPC_INVALID_PARAMETER,
                               ...
                               "(funding|valid|delete|endorsed) OR `custom oversight code` ");
        }
        ...
        if(!fMnFound) {
            ...
            resultsObj.push_back(Pair("aeriscoin.conf", statusObj));
...
UniValue getgovernanceinfo(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() != 0) {
        throw std::runtime_error(
            ...
            "  \"masternodewatchdogmaxseconds\": xxxxx,  (numeric) oversight watchdog expiration time in seconds\n"
