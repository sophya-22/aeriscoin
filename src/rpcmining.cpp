...
UniValue prioritisetransaction(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() != 3)
        throw runtime_error(
            ...
            "                  (priority of a transaction is calculated: coinage * value_in_Aenits / txsize) \n"
            "3. fee delta      (numeric, required) The fee value (in Aenits) to add (or subtract, if negative).\n"
...
UniValue getblocktemplate(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() > 1)
        throw runtime_error(
            ...
            "             ,...\n"
            "         ],\n"
            "         \"fee\": n,                   (numeric) difference in value between transaction inputs and outputs (in Aenits); for coinbase transactions, this is a negative Number of the total collected block fees (ie, not including the block subsidy); if key is not present, fee is unknown and clients MUST NOT assume there isn't one\n"
            ...
            "      \"flags\" : \"flags\"            (string) \n"
            "  },\n"
            "  \"coinbasevalue\" : n,               (numeric) maximum allowable input to coinbase transaction, including the generation award and transaction fees (in Aenits)\n"
    ...
    if (vNodes.empty())
        throw JSONRPCError(RPC_CLIENT_NOT_CONNECTED, "Aeriscoin Core is not connected!");
    /*
        if (IsInitialBlockDownload())
            throw JSONRPCError(RPC_CLIENT_IN_INITIAL_DOWNLOAD, "Aeriscoin Core is downloading blocks...");

        if (!masternodeSync.IsSynced())
            throw JSONRPCError(RPC_CLIENT_IN_INITIAL_DOWNLOAD, "Aeriscoin Core is syncing with network...");
    */
